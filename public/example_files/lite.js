...
function psr(html) {
  ...
  html = html.replace(/http:\/\/cs(\d+)\.(userapi\.com|vk\.com)\/([a-z0-9\/_:]+\.jpg)/gi, 'https://pp.userapi.com/c$1/$3');
  html = html.replace(/http:\/\/cs(\d+)\.(userapi\.com|vk\.com)\//gi, 'https://ps.userapi.com/c$1/');
  html = html.replace(/http:\/\/video(\d+)\.vkadre\.ru\//gi, 'https://ps.userapi.com/v$1/');
  return html;
}
...
function __adsSet(adsHtml, adsSection, adsCanShow) {
  __adsSet = function() {
    window.AdsLight && AdsLight.setNewBlock.apply(AdsLight.setNewBlock, arguments);
  };
  stManager.add(['aes_light.js'], __adsSet.pbind(adsHtml, adsSection, adsCanShow));
}
  ...
  _post: function(url, q, o) {
    ...
    var processResponse = function(code, answer) {
      ...
      case 2: // captcha
        ...
        if ( o._captcha.bodyNode && (o._suggest = geByClass1('phone_validation_link', o._captcha.bodyNode))) {
      ...
      default:
        if (code == -1) {
          ...
          __adsSet(adsHtml, null, adsCanShow);
        }
